    ...
    <div className="space-y-7 pt-2 pb-12 px-6">
      ...
      <div className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <Button size="sm" className="h-9 text-xs font-bold gap-1.5 shadow-sm px-5">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <KPICard
          ...
          data={[40, 55, 45, 70, 65, 85, 92]}
        />
        <KPICard
          title="OneTicket Holders"
          value="18,350"
          change="+8.2%"
          changeLabel="242 new today"
          up
          icon={<Users className="h-4 w-4" />}
          accent="from-violet-500/20 to-violet-500/5"
          data={[30, 40, 35, 50, 48, 60, 65]}
          color="#8b5cf6"
        />
        <KPICard
          ...
          <CardFooter className="border-t border-border/50 px-6 py-3 flex items-center justify-between bg-muted/10">
            <div className="flex items-center gap-4">
              <div className="text-center">
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Week Revenue</p>
                <p className="text-sm font-bold text-foreground mt-0.5">₦42.2M</p>
              </div>
              <div className="w-px h-8 bg-border" />
              <div className="text-center">
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">New Users</p>
                <p className="text-sm font-bold text-foreground mt-0.5">1,290</p>
              </div>
              <div className="w-px h-8 bg-border" />
              <div className="text-center">
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground font-semibold">Peak Day</p>
                <p className="text-sm font-bold text-foreground mt-0.5">Sunday</p>
              </div>
            </div>
            <Button variant="ghost" size="sm" className="text-xs font-bold gap-1 text-primary h-8">
          ...
          <CardContent className="p-0">
            <div className="divide-y divide-border/40">
              {recentRegistrations.map((reg, idx) => (
                <div key={idx} className="flex items-center justify-between px-5 py-3.5 hover:bg-muted/30 transition-colors cursor-pointer group">
                  <div className="flex items-center gap-3">
                    <Avatar className="h-9 w-9 border-2 border-border/60 group-hover:border-primary/40 transition-colors shrink-0">
                      <AvatarFallback className={`${reg.color} text-white text-[10px] font-bold`}>
                        {reg.name.split(' ').map(n => n[0]).join('')}
                      </AvatarFallback>
                    </Avatar>
                    <div>
                      <p className="text-sm font-bold leading-tight">{reg.name}</p>
                      <div className="flex items-center gap-1.5 mt-0.5">
                        <span className={`text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full ${reg.role === 'Driver' ? 'bg-sky-500/10 text-sky-700' : 'bg-violet-500/10 text-violet-700'}`}>
                          {reg.role}
                        </span>
                        <span className="text-[10px] font-mono text-muted-foreground">{reg.id}</span>
                      </div>
                    </div>
                  </div>
                  <div className="text-right shrink-0">
                    <p className="text-[10px] text-muted-foreground font-medium">{reg.time}</p>
                    <Badge
                      className={`text-[9px] font-bold border-none px-2 h-4 mt-1 ${
                        reg.status === 'Verified' ? 'bg-emerald-500/10 text-emerald-700' :
          ...
          <Card className="border-border/60 shadow-sm bg-card/80">
            <CardHeader className="pb-3 flex flex-row items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <CardTitle className="text-sm font-bold">Active Risks</CardTitle>
              </div>
              <Badge className="text-[9px] font-bold border-none bg-rose-500/10 text-rose-600 h-5">3 Open</Badge>
            </CardHeader>
            <CardContent className="space-y-2.5 pt-1">
              {risks.map((r, i) => (
                <div key={i} className="flex items-center justify-between p-3 rounded-xl bg-muted/30 border border-border/40 hover:border-rose-500/20 transition-all cursor-pointer group">
                  <div className="min-w-0">
                    <p className="text-xs font-bold leading-tight truncate">{r.label}</p>
                    <p className="text-[10px] text-muted-foreground font-mono mt-0.5">{r.route} · {r.time}</p>
                  </div>
                  <Badge className={`ml-3 shrink-0 text-[9px] font-bold border-none h-5 ${
                    r.risk === 'Critical' ? 'bg-rose-500 text-white' :
                    r.risk === 'High' ? 'bg-rose-500/10 text-rose-600' :
          ...
          <Card className="border-border/60 bg-zinc-950 text-white shadow-xl relative overflow-hidden">
            ...
            <CardContent className="pt-5 space-y-3 relative z-10">
              <div className="grid grid-cols-2 gap-2">
                ...
                <button className="flex items-center justify-center gap-1.5 border border-white/10 bg-white/5 hover:bg-white/10 transition-all rounded-xl px-3 py-2.5 text-[11px] font-bold text-white">
                  <UserCheck className="h-3.5 w-3.5" /> New User
                </button>
              </div>
              <button className="w-full flex items-center justify-center gap-2 bg-primary hover:bg-primary/90 text-white font-bold text-sm rounded-xl py-2.5 transition-all shadow-lg shadow-primary/20">
                <Zap className="h-4 w-4" /> Dispatch Ops Centre
              </button>
            </CardContent>
